...
public class SpiderProceduralAnimation : MonoBehaviour
{
    ...
    IEnumerator PerformStep(int index, Vector3 targetPoint)
    {
        ...
        for(int i = 1; i <= smoothness-2; ++i)
        {
            legTargets[index].position = Vector3.Lerp(startPos, targetPoint, i / (float)(smoothness));
            legTargets[index].position += transform.up * Mathf.Sin(i / (float)(smoothness) * Mathf.PI) * stepHeight;
